...
import { fetchPostTaxs } from "appRedux/actions/staff";
...
class PostTax extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    this.props.fetchPostTaxs();
  }

  render() {
    const { Option } = Select;
    const { postTaxDeductions } = this.props;
    ...
    let dataSource = postTaxDeductions.length
      ? postTaxDeductions.map((item) => {
          return {
            key: item.id,
            id: item.id,
    ...
    return (
      <section className="leavepolicy-section container">
        <Divider orientation="left">Post Tax Deduction</Divider>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...bindActionCreators({ fetchPostTaxs }, dispatch),
  };
};
...
const mapStateToProps = (state) => {
  return {
    ...
    postTaxDeductions: state.staffs.postTaxDeductions,
    loader: state.staffs.loader,
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(PostTax);
